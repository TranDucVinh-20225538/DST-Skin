...
def main():
    ...
    print(f" Extracting features/logits with Resnet50 on {DEVICE}")

    # 1. Model
    model = get_resnet50(num_classes=2, pretrained=False)
    state = torch.load("data/models/resnet50_robust_best.pth",
                       map_location=DEVICE)
    ...
    save_path = "outputs/features/resnet50_isic_pad_features.pt"
    torch.save({
