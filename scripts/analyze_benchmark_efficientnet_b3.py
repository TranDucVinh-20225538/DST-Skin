...
def main():
    ...
    train_labels = to_numpy(data["train_labels"])  # không dùng trong OODScorer nhưng giữ lại
    ...
    # cần có trong file .pt, đã thêm từ script extract
    ...
    # 2. Fit OODScorer một lần trên toàn bộ train_feats (ReAct + ViM)
    ...
    # =========================
    print("\n=== Score comparison (EffNet-B3, OODScorer + ViM/ReAct) ===")
    ...
    df_scores.to_csv("outputs/reports/effb3_score_comparison.csv", index=False)
    ...
    df_disp.to_csv(
        "outputs/reports/effb3_dispersion_vs_separability.csv", index=False
    )
    print("\n=== Dispersion vs separability (EffNet-B3) ===")
    ...
    for n in n_list:
        ...
        # Fit một OODScorer mới trên subset (không cần ViM/ReAct ở ablation này)
    ...
    df_ablation.to_csv("outputs/reports/effb3_mahalanobis_ablation_N.csv", index=False)

    # =========================
    # 5.3 RISK-COVERAGE (Mahalanobis)
    # =========================
    print("\n=== Risk-Coverage data (EffNet-B3, Mahalanobis via OODScorer) ===")

    # Dự đoán nhãn ID val từ logits (để biết đúng/sai)
    ...
    df_rc.to_csv("outputs/reports/effb3_risk_coverage_mahalanobis.csv", index=False)
    ...
    df_rc_summary.to_csv(
        "outputs/reports/effb3_risk_coverage_summary.csv", index=False
    )
    ...
    print("\n✅ Saved files:")
    print(" - outputs/reports/effb3_score_comparison.csv")
    print(" - outputs/reports/effb3_dispersion_vs_separability.csv")
    print(" - outputs/reports/effb3_mahalanobis_ablation_N.csv")
    print(" - outputs/reports/effb3_risk_coverage_mahalanobis.csv")
    print(" - outputs/reports/effb3_risk_coverage_summary.csv")
