...
def main():
    ...
    print(f" Extracting features/logits with EfficientNet-B3 on {DEVICE}")

    # 1. Model
    model = get_efficientnet_b3(num_classes=2, pretrained=False)
    state = torch.load("data/models/efficientnet_b3_robust_best.pth",
                       map_location=DEVICE)
    ...
    fc_weight = model.classifier[1].weight.detach().cpu()
    fc_bias   = model.classifier[1].bias.detach().cpu()

    save_path = "outputs/features/effb3_isic_pad_features.pt"
    torch.save({
