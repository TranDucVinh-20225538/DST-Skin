# scripts/run_journal_benchmark.py
...
def get_dataloader(split: str, batch_size: int = 64):
    ...
    Bạn chỉnh lại đường dẫn CSV / IMG đúng với project.
    """
    tfm = build_transform()
    ...
    if split == "isic_train_subset":
        ...
        # lấy ngẫu nhiên 5000 mẫu để fit Maha/kNN như code cũ
        np.random.seed(42)
...
def run_benchmark():
    ...
    # 2. MODEL (ví dụ: ResNet-18 Robust)
    model = get_resnet18(num_classes=2)
    model.load_state_dict(torch.load("data/models/resnet18_robust_epoch10.pth",
                                     map_location=device))
    model.to(device)
    ...
    out_path = "outputs/results_resnet18_robust.csv"
