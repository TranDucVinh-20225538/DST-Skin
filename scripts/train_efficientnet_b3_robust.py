...
def build_train_transform():
    # Heavy augmentation giống các mô hình CE-Robust trước
    return transforms.Compose([
        transforms.Resize(300),                   # B3 input 300x300
...
def get_dataloaders(batch_size=32, num_workers=0):
    # B3 nặng hơn, nên batch_size nhỏ hơn 64
...
def train_efficientnet_b3_robust():
    os.makedirs("data/models", exist_ok=True)

    device = torch.device("mps" if torch.backends.mps.is_available()
                          else "cuda" if torch.cuda.is_available()
                          else "cpu")
    print(f"🚀 Training EfficientNet-B3 CE-Robust on {device}")

    train_loader, val_loader = get_dataloaders(batch_size=32, num_workers=0)

    model = get_efficientnet_b3(num_classes=2, pretrained=True).to(device)
    ...
    best_val_auc = 0.0
    best_path = "data/models/efficientnet_b3_robust_best.pth"
    ...
    final_path = "data/models/efficientnet_b3_robust_epoch_last.pth"
    torch.save(model.state_dict(), final_path)
    print(f"💾 Saved final model to {final_path}")


if __name__ == "__main__":
    train_efficientnet_b3_robust()
